...
export function FirebaseProvider({ children }) {
  ...
  const [currentSearchIntent, setCurrentSearchIntent] = useState(null);
  ...
  const searchIntentListenerRef = useRef(null);
  ...
  const trackSearchIntent = (docId, url) => {
    if (!docId) return;
    // If already tracking the same analysis, do nothing.
    if (currentSearchIntent && currentSearchIntent.docId === docId) return;

    // Unsubscribe previous listener, if any.
    if (searchIntentListenerRef.current) {
      searchIntentListenerRef.current();
      searchIntentListenerRef.current = null;
    }

    // Set initial analysis state.
    setCurrentSearchIntent({
      docId,
      url,
      type: "search-intent",
      status: "pending",
    ...
    // Set up a listener for the analysis document.
    searchIntentListenerRef.current = onSnapshot(doc(db, "searchIntent", docId), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        setCurrentSearchIntent((prev) => ({
          ...prev,
  ...
  const removeSearchIntent = () => {
    if (searchIntentListenerRef.current) {
      searchIntentListenerRef.current();
      searchIntentListenerRef.current = null;
    }
    setCurrentSearchIntent(null);
  };
