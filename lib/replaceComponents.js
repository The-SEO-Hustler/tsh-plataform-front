    ...
    replace: ({ attribs, name, children }) => {
      ...
      if (name === 'div' && attribs?.id === 'faq-tsh') {
        ...
        for (const child of children) {
          if (child.type === 'tag' && child.name === 'h2' && child.attribs?.class === 'faq-block-title') {
            title = domToReact(child.children)
          }
          if (child.type === 'tag' && child.name === 'p' && child.attribs?.class === 'faq-block-desc') {
        ...
        const items = (listWrapper?.children || [])
          .filter(c => c.type === 'tag' && c.name === 'div' && c.attribs?.class === 'faq-item')
          .map(item => {
            const qNode = item.children.find(
              c => c.type === 'tag' && c.name === 'h3' && c.attribs?.class === 'faq-q'
            )
            const aNode = item.children.find(
              c => c.type === 'tag' && c.name === 'div' && c.attribs?.class === 'faq-a'
            )
            return {
      ...
      if (
        ...
        const codeSpan = children.find(
          (c) =>
            c.type === 'tag' &&
            c.attribs?.['data-code'] !== undefined
        )

        const codeText = codeSpan
          ? codeSpan.attribs?.['data-code']
          : ''
        ...
        return (
          ...
          <CodeBlock language={language === 'Plaintext' ? 'text' : language} code={codeText} />
        )
      }
