...
import EmbedTweet from "@/components/embed-tweet";
        ...
        if (domNode.attribs?.class?.includes("wp-block-image")) {
          ...
          if (domNode.children[0].name === "img") {
            ...
            const props = transformProps(
              attributesToProps({ ...element.attribs, isPriority })
            );
            const { className: imgClassName, ...otherImgProps } = props;
            const captionProps = transformProps(
              attributesToProps(element?.next?.attribs)
            );
            const { className: captionClassName, ...otherCaptionProps } =
              captionProps;
            ...
            return caption ? (
              <figure>
                <Component
                  className={clsx("my-0", imgClassName)}
                  {...otherImgProps}
                />
                <figcaption
                  className={clsx(
          ...
          if (element.name === "a" && element.children[0].name === "img") {
            const linkProps = transformProps(
              attributesToProps(element?.attribs)
            );
            const { href: linkHref, ...otherLinkProps } = linkProps;
            const imgProps = transformProps(
              attributesToProps(element?.children[0]?.attribs)
            );
