...
export default function LoadingScreenSearchIntent({ status = 'pending' }) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center ">
      <div className="text-center md:max-w-md p-8 bg-card rounded-xl shadow-lg max-w-[90%]">
        ...
        <div className="mt-8 text-xs text-foreground/80 flex items-center gap-2">
          ...
          <button onClick={() => {
            navigator.clipboard.writeText(`${process.env.NEXT_PUBLIC_FRONT_URL}/search-intent/result?id=${searchParams.get('id')}`)
            toast.success("Link to search intent copied to clipboard")
          }
          }
