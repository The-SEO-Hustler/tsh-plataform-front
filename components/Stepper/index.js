...
export default function Stepper({
  ...
  useEffect(() => {
    if (!autoAdvance || isCompleted) return;

    const interval = setInterval(() => {
      if (isLastStep) {
        setDirection(1);
        updateStep(totalSteps + 1);
        // Reset to first step after a short delay for continuous loop
        setTimeout(() => {
          setCurrentStep(1);
        }, 1000);
      } else {
  ...
  }, [autoAdvance, autoAdvanceInterval, currentStep, isLastStep, isCompleted, totalSteps]);
